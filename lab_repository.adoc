...
The purpose of this document is to describe the basic features that the "Core" Service will include for the Virtual Research Lab Readiness Framework. This service should be implemented as an independently as a microservice, and any further complexity should be focused on different services that will be built on top of it.
...
| HA2
| NaaVRE is going through a major refactoring, and any integration should be done with the new architecture
|===
...
[TIP]
Virtual Labs (VL) allows scientists to experiment with large datasets, and verify other experiments accordingly given the correctness of each Lab.
====
Each VL has several quality properties that can act as conditions to signal the _maturity_ of each Lab.
====
====
...
| LR3
| The Request can be approved/declined by the VRE Coordinator
|===
...
| LRR1
| Each VL should maintain the current _readiness_ level
...
| LRR6
| The golden user and the Component/Services Developer can request a change of the readiness level
|===
...
| ORR1
| Access should be provided to external systems and update the _exit conditions_ programatically

| ORR2
| The public data about the Labs should be visible without the need of authentication
...
### Proposed Models
The main entity of this application is the Virtual Lab. The following models are proposed to be used in order to represent the Virtual Labs and their readiness levels.
...
Aditionally, the exit condition entity can have a category or type enum to indicate the type and allow easier filtering when displaying or managing exit conditions.

##### Metadata
The exit condition entity can contain several fields as metadata to improve usability.

- *Comments*: Comments can be added, providing additional context or information about the exit condition.
- *Discussion URL*: URL to an external discussion thread, e.g. a ticket in a Project Management tool.
- *Tooltip URL*: This can be a link with generic information related to the specific exit condition.
- *Created/Updated At*: Timestamps indicating when the exit condition was created or last updated.
- *Created/Updated By*: Usernames or IDs indicating who created or last updated the exit condition.
